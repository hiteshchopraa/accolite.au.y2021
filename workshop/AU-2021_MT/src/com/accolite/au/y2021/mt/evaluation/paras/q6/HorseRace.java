...
class HorseRace extends Thread {
	...
	int gap;
	static int num;
	static int distance;// i have taken distance to be covered by horses as 1000 , we can also ask user
						// to provide this
	...
	public void run() {
		do {// do while block is used so that a single horse thread can keep on running till
			...
			synchronized (this)// synchronized so that only one thread can manipulate it at a time
			{
				...
				Statics s = new Statics(horse, race.getName(), gap);// calling thread so that we can update the horse
																	// object acc to the speed we got above
				s.start();
